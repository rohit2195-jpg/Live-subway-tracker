...
This is a live NYC subway Tracker. All lines have been added! 
## Overview  
...
## Difficulties
The program currently tracks 70-73% of all trains in the NYC subway system. This limitation stems from discrepancies between the real-time data provided by the subway system and the pre-scheduled train times, especially when temporary service changes are not reflected in the schedule. Additionally, some of the data is inaccurate and fails to match the static datasets for each train’s schedule. Specifically, certain records contain invalid trip_id values.

Relying solely on the API proved insufficient for tracking all trains, as the available data was too limited. For instance, the API did not provide latitude and longitude coordinates for each train. As a result, I had to implement linear interpolation, which required incorporating additional information from the pre-scheduled train data.
